...
import vtcac.thuhuong.mytrip.utils.MyString;
...
public class EditTravelActivity extends BaseActivity implements View.OnClickListener, DatePickerDialog.OnDateSetListener {
        ...
        switch (view.getId()) {
            ...
            case R.id.place_et: {
                ...
                try {
                    ...
                    // Indicate that Google Play Service is either not installed
                    // or not up to date
                    // Prompt the user to connect the issue
                    GoogleApiAvailability.getInstance().getErrorDialog(this,
                            e.getConnectionStatusCode(),
                            0/*request code*/).show();
                } catch (Exception e) {
                    // Indicate that Google Play Service is not available
                    // and the problem is not easily resolvable.
                    String message = "Google Paly Service is not available: " +
                            e.getMessage();
                    Log.e(TAG, message);
    ...
    private void validate() {
        ...
        if (mStartDt == 0) {
            Snackbar.make(mTitleEt, R.string.start_date_empty, Snackbar.LENGTH_SHORT).show();
            return;
        }
        if (mEndDt == 0) {
            Snackbar.make(mTitleEt, R.string.end_date_empty, Snackbar.LENGTH_SHORT).show();
            return;
        }
        // add a new travel
        if (mTravel == null) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (minEditMode) {
            getMenuInflater().inflate(R.menu.menu_edit_travel, menu);
        }
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_travel_del:{
                showAlertOkCancel(R.string.alert_title_delete_activity
                        , R.string.alert_message_delete_activity,
